...
type Client interface {
	BurnEvent(ctx context.Context, asset multichain.Asset, nonce pack.Bytes32, blockheight pack.U64) (amount pack.U256, to pack.String, confs int64, err error)
}
...
func NewClient(rpcUrl string) (Client, error) {
	substrateAPI, err := gsrpc.NewSubstrateAPI(rpcUrl)
	if err != nil {
		return nil, err
	}
...
func (client client) BurnEvent(ctx context.Context, asset multichain.Asset, nonce pack.Bytes32, blockheight pack.U64) (amount pack.U256, to pack.String, confs int64, err error) {
	...
	meta, err := client.RPC.State.GetMetadataLatest()
	if err != nil {
		panic(err)
	}
	...
	blockhash, err := client.RPC.Chain.GetBlockHash(blockheight.Uint64())
	if err != nil {
		panic(err)
	}
	...
	data, err := client.RPC.State.GetStorageRaw(key, blockhash)
	...
	sub, err := client.RPC.State.SubscribeStorageRaw([]types.StorageKey{key})
	if err != nil {
		panic(err)
	}
