...
func FindEventForEventID(m types.MetadataV11, eventID types.EventID) (*types.EventMetadataV4, error) {
	mi := uint8(0)
...
func DecodeEvents(e *types.EventRecordsRaw, m *types.Metadata, t interface{}) error {
	...
	for i := uint64(0); i < n.Uint64(); i++ {
		...
		if !field.IsValid() {
			eventParams, err := FindEventForEventID(m.AsMetadataV11, id)
			if err != nil {
				return fmt.Errorf("unable to find event with EventID %v in metadata for event #%v: %s", id, i, err)
			}
